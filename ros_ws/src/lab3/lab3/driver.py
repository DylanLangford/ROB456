...
class Lab3Driver(Node):
  ...
  # YOUR CODE HERE
		self.obstacle_distance = 0.35          # meters
		self.front_angle = 50.0 * pi / 180.0  # +/- 20 degrees
  ...
  # YOUR CODE HERE
		...
		if min_dist < self.obstacle_distance:
			# Obstacle detected — stop and turn left
  ...
  # YOUR CODE HERE
		...
		#                         self.target.point.x)
		# if angle_to_target < -0.3:
		# 	t.twist.angular.z = -max_turn
		# elif angle_to_target > 0.3:
		# 	t.twist.angular.z = max_turn
		# else:
		# 	t.twist.angular.z = 0
		...
		if not self.avoiding:
			if fabs(angle_to_target) < pi / 20.0:
		...
		if not obstacle and self.avoiding:
